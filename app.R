...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      sliderInput("alcoholInput", "Alcohol Content", 0, 100, c(0, 100), pre = "%"),
      uiOutput("typeSelectOutput"),
      checkboxInput("filterSweetness", "Filter by sweetbess", FALSE),
      conditionalPanel(
        condition = "input.filterSweetness",
        uiOutput("sweetSelectOutput")
      ),
      ...
      checkboxInput("plotSweetness", "Plot the sweetness", FALSE),
...
server <- function(input, output, session) {
  ...
  output$sweetSelectOutput <- renderUI({
    selectInput("sweetInput", "Product Sweetness",
  ...
  prices <- reactive({
    ...
    if (input$filterSweetness) {
      prices <- dplyr::filter(prices, Sweetness == input$sweetInput)
    }
    ...
    prices <- dplyr::filter(prices, Alcohol_Content >= input$alcoholInput[1],
                            Price <= input$alcoholInput[2])
    if(nrow(prices) == 0) {
      return(NULL)
    }
    prices
  ...
  output$plot <- renderPlot({
    ...
    if (input$plotSweetness) {
      ggplot(prices(), aes(Sweetness, fill = Type)) +
